        ...
        def follow(i):
            ...
            try:
                ...
                for i in range(len(tokens_data)):
                    try:
                        token_id, token, token_username, email, e_code = random.choice(tokens_data).split("|")
                ...
                proxie = "http://"+random.choice(self.proxies)
                res = httpx.post('https://gql.twitch.tv/gql', data=payload, headers=headers, proxy= proxie,timeout=40)
                print(res.text)
